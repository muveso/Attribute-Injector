    ...
    public static class MethodEditor
    {
        ...
        public static void BeforeExit(this MethodBody body, PointCut action)
        {
            foreach (var ret in body.Instructions.Where(i => i.OpCode == OpCodes.Ret).ToList())
            {
                ...
                cut.Next().Here(action).Write(OpCodes.Ret);
                cut.Remove();
            }
        }

        public static void BeforeAfter(this MethodBody body, int kind, PointCut action)
        {
            if (kind == 1)
                AfterEntry(body, action);
            else
                BeforeExit(body, action);
        }
